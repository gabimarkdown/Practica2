...
#Creacion del objeto ggplot sobre sus dos variables indicadas en el apartado y el grafico de dispersion
grafico1<-ggplot(ds, aes(x = MinTemp, y = MaxTemp))+geom_point()

#Le añado la variable categorica RainTomorrow a traves del color 
grafico2<-ggplot(ds, aes(x = MinTemp, y = MaxTemp,colour=RainTomorrow))+geom_point()

#Le añado el titulo al grafico que tenia del apartado anterior
grafico3<-ggplot(ds, aes(x = MinTemp, y = MaxTemp,colour=RainTomorrow))+geom_point()+ggtitle("Ggplot ejercicio 1")
grafico3

#guardo el grafico en formato pdf
ggsave("Ejercicio1.pdf",device="pdf")
```
...
#grafico de dsipersion sobre las variables indicadas
grafico<-ggplot(ds, aes(x = Temp9am, y = Pressure9am))+geom_point()
grafico

#le he incluido el color de los puntos sobre la variable categorica
grafico_1<-ggplot(ds, aes(x = Temp9am, y = Pressure9am,shape=RainToday))+geom_point()
grafico

#le he incluido la forma del punto sobre la variable categorica y le añafo un titulo descriptivo
grafico_2<-ggplot(ds, aes(x = Temp9am, y = Pressure9am,colour=RainToday))+geom_point()+ggtitle("Grafico sobre dos variables(Temp9am y Pressure9am)de un conjunto de datos(ds)")
grafico_1

#La informacion se observa mejor con la representacion color ya que hace que la grafica se vea mas clara y los datos mejor definidos

#guardo el ultimo grafico
ggsave("Ejercicio2.pdf",device="pdf")
```
...
#cargo el archivo con load
load("./data/iris_tidy.Rdata")
#estructura del objeto R guardado en el archivo
str(iris.tidy)

#objeto ggplot con sus respectivas variables
...
#visualizo la variable part con colour
iris.grafico2<-ggplot(iris.tidy, aes(x = Species, y = Value,colour=Part))+geom_point()
iris.grafico2

#Introduzco una tercera variable measure en forma de filasy columnas
iris.grafico2 + facet_grid(Measure~. )
iris.grafico2  + facet_grid(.~Measure )
#la disposicion en columnas permite compararlas mejor ya que se ven mas claros y definidos los datos
iris.grafico3<-ggplot(iris.tidy, aes(x = Species, y = Value,colour=Part))+geom_jitter()
iris.grafico3
#Observo que los graficos se dividen en tres zonas y los puntos estan mas separados no como en el grafico original
...
library(ggplot2)
#examinar las variables del dataset
str(DatosEjer4)

#grafico de puntos sobre las variables fecha y ventas y el color fabricante
ggplot(DatosEjer4, aes(x=Fecha, y=Ventas,colour=Fabricante))+
geom_point()
#este grafico no es valido para representar la evolucion de las ventas de cada fabricante ya que no es claro y no nos indica una linea clara de progresion a la que seguir ,algo que si tendriamos mediante una linea.

#le incluyo la linea
ggplot(DatosEjer4, aes(x=Fecha, y=Ventas,colour=Fabricante))+
geom_point()+geom_line()

#cambio el tipo de linea de cada tendencia de cada fabricante y añado el titulo
ggplot(DatosEjer4, aes(x=Fecha, y=Ventas,colour=Fabricante))+
geom_point()+geom_line(aes(linetype=Fabricante))+ggtitle("Ventas mensuales de diferentes compañías tecnológicas")

#almacenar
ggsave("Ejercicio4.pdf",device="pdf")
```
...
#graficon de puntos incluyendo lineas distintas y con diferente trazo y formas de punto distintas
ggplot(Experimento, aes(x=Dosis, y=Altura,group=Complemento,linetype=Complemento,shape=Complemento))+
geom_point()+geom_line()

#añado las barras de error
ggplot(Experimento, aes(x=Dosis, y=Altura,group=Complemento,linetype=Complemento,shape=Complemento))+geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))+
geom_point()+geom_line()

#cambio el sitio del tipo de linea y forma del punto y añado la paleta de colores rojos
p<-ggplot(Experimento, aes(x=Dosis, y=Altura,group=Complemento,color=Complemento))+geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))+
geom_point(aes(shape=Complemento))+geom_line(aes(linetype=Complemento))


p +theme_classic() + scale_color_brewer(palette="Reds")

#almacenar
ggsave("Ejercicio5.pdf",device="pdf")
...
#boxplots de TempMin para cada uno de los años de Year
ggplot(ds)+geom_boxplot(aes(x=MinTemp,y=Year))

#boxplots violin
grafico<-ggplot(ds)+geom_violin(aes(x=MaxTemp,y=Year,colour=Location))

#añadir etiquetas
grafico_1<-grafico + labs(title="Distribución de temperaturas máximas por año",x="Año",y="Temperatura (ºC)")
grafico_1

#quitamos leyenda,para eso tenemos que añadir un identificativo a cada uno de los boxplots para que exista la leyenda,en forma de color,relleno...,cosa que habre hecho en el primer boxplots violin


grafico_1+ theme(legend.position="none")+theme(axis.text.x=element_text(angle=45))

#almacenar
ggsave("Ejercicio6.pdf",device="pdf")
```
...
chol_1<-read.table("http://assets.datacamp.com/blog_assets/chol.txt",header=TRUE)

#representacion del histograma
chol_2<-ggplot(chol_1, aes(x=HEIGHT)) + geom_histogram()

#añado los titulos
chol_2+labs(title="Histograma de la variable Altura",x="Altura (cm)",y="Frecuencia")

#ajusto el grado de representacion y cambiar el numero de bins a 20
ggplot(chol_1, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155,195,by=20))+labs(title="Histograma de la variable Altura",x="Altura (cm)",y="Frecuencia")

#color,transpariencia y contorno de las barras
ggplot(chol_1, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155,195,by=20),col="red",fill="blue",alpha= .5)+labs(title="Histograma de la variable Altura",x="Altura (cm)",y="Frecuencia")

#añado curva de distribucion

ggplot(chol_1, aes(x=HEIGHT)) +
geom_histogram(aes(y =stat(density)),
breaks=seq(155,195, by = 20),
col="red",
fill="blue",
alpha=.5) +
geom_density(col="green") +
labs(title="Histograma de la variable Edad", x="Altura(cm)", y="Frecuencia")
...
alpha=.5) +
geom_density(col="green") +
labs(title="Histograma de la variable Edad", x="Altura(cm)", y="Frecuencia")+facet_grid(SMOKE~. )

ggsave("Ejercicio7.pdf",device="pdf")
